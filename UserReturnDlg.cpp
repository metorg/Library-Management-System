...
BOOL UserReturnDlg::OnInitDialog()
{
	...
	m_list.InsertColumn(0, _T("도서 제목"), LVCFMT_LEFT, int(rect.Width() + 0.4), -1);
	...
	m_list.InsertColumn(4, _T("대여날"), LVCFMT_LEFT, int(rect.Width() + 0.3), -1);
	m_list.InsertColumn(5, _T("반납 기한"), LVCFMT_LEFT, int(rect.Width() + 0.3), -1);
...
void UserReturnDlg::ReturnDB()
{
	...
	for (int i = 0; i < bookreturn.size(); i++)
	{
		...
		for (int j = 0; j < bookreturn[i].GetLength(); j++)
		{//날짜계산
			if (j < 4)
			{
				year = (year * 10) + (bookreturn[i][j] - '0');
			}
			else if (4 < j && j < 7)
			{
				month = (month * 10) + (bookreturn[i][j] - '0');
			}
			else if (7 < j)
			{
				day = (day * 10) + (bookreturn[i][j] - '0');
			}
		}
		//일수 계산 함수
	...
	while (!bookname.empty())
	{
		...
		{//현재 날짜보다 반납 날짜가 적으면 메시지창
			MessageBox(_T("반납기한을 초과하셨습니다"));
		}
		else
		{
			query.Format(_T("DELETE FROM rental WHERE title = '%s'"), bookname.back());
